...
use frame_support::{
	...
	traits::{
		CallerTrait, Currency, ExistenceRequirement::KeepAlive, Get, OriginTrait, Randomness,
		ReservableCurrency,
	},
	PalletId,
};
...
use xcm::latest::{ExecuteXcm, Parent};
use xcm::opaque::v3::{Junction::AccountId32, Junctions::Here};
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config + pallet_xcm::Config {
		...
		type RuntimeCall: Parameter
			+ Dispatchable<RuntimeOrigin = <Self as pallet::Config>:::RuntimeOrigin>
			+ GetDispatchInfo
		...
		/// The manager origin.
		type ManagerOrigin: EnsureOrigin<<Self as pallet::Config>::RuntimeOrigin>;
		...
		#[pallet::constant]
		type RelayChainTreasuryAccount: Get<SpAccountId32>;
...
impl<T: Config> Pallet<T> {
	...
	/// Prototype function for teleport assets to relay chain
	pub fn xcm_transfer(origin: T::PalletsOrigin, amount: u128) {
		let treasury_acc = T::RelayChainTreasuryAccount::get();
		let account_bytes: [u8; 32] = treasury_acc.into();

		pallet_xcm::Pallet::<T>::teleport_assets(
			origin,
			Box::new(Parent.into()),
			Box::new(AccountId32 { network: None, id: account_bytes }.into()),
			Box::new((Here.into(), amount).into()),
			0,
		);
	}
}
